...
- 这时，我们就应采用保守学习的思想，考虑最坏情况，获得一个相对松弛的上界 (_uniform bound_)
  $$
  P\big(\exist h \in \mathcal{H}, \big| \hat{\varepsilon}_{D_n}(h) - \varepsilon(h) \big | \geq \epsilon\big)  = P\big(\sup_{h\in \mathcal{H}} \big|  \hat\varepsilon_{D_n}(h) - \varepsilon(h) \big | \geq \epsilon \big|\big)\quad (1)
  \\
